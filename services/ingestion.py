...
import pandas as pd
from services.s3 import upload_file_to_s3, download_to_bytes
import hashlib
import re
from datetime import date
import csv
from io import StringIO, BytesIO
from services.tables import prepare_ingestion_artifacts
...
def ingest_file_pipeline(file: UploadFile):
    ...
    checksum, period, file_id = prepare_ingestion_artifacts(file_name, buffer)
...
def ingest_buffer_pipeline(buffer: BytesIO, file_name: str,versionID: str = None):
    """
    ...
    """
    checksum, period, file_id = prepare_ingestion_artifacts(file_name, buffer=buffer)
...
def ingest_multiple_s3_keys(s3_keys: List[str]):
    """
    Ingest multiple files from S3 given their keys.
    Returns a list of ingestion results.
    """
    results = []
    for key in s3_keys:
        buffer,versionID = download_to_bytes(key)  # will raise ValueError if duplicate
        res = ingest_buffer_pipeline(buffer, Path(key).name,versionID)
        results.append(res)
    return results